...
4. **Pass 3 (career sites)** - Scrapes the Big Tech career sites directly and keeps postings from the last seven days; postings without a reliable date are retained, and date-only postings are kept when their calendar day overlaps the window. Amazon's score-ranked results are paginated until enough qualifying jobs are found or the source is exhausted.
